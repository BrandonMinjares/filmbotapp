...
# should only select users with 15 reviews
...
def user_average_rating(userID, commonItem):
        avg_rating = 0.0
        for x in commonItem:
            # print(data.iloc[userIDS[userID]][x])
                # print(data.iloc[userIDS[userID]][x])
...
for x in range(0, len(data.index)):
    user2 = data.iloc[x][0]
    correlationsArr.append(pearson_correlation('f4c2714f-6888-485c-a6e0-3508d4fc77b9', user2))
...
for x in range(1, len(data.columns)):
    ...
    if numerator == 0 or denominator == 0:
        ...
        predictedScores.append(userFindAvgById['f4c2714f-6888-485c-a6e0-3508d4fc77b9'] + score)

# print(sorted(predictedScores))
print(sorted(predictedScores))
topMovies = sorted(range(len(predictedScores)), key=lambda i: predictedScores[i])[-7:]
print('top movies')
print(topMovies)
...
cur.execute(sql, (recommendations, 'f4c2714f-6888-485c-a6e0-3508d4fc77b9'))
# close the cursor and connection objects
conn.commit()
cur.close()
conn.close()